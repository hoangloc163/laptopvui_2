...
# ---------- Sanity check: file source có đầy đủ không? ----------
# Fail sớm với message rõ ràng nếu repo thiếu file quan trọng (thường do
# upload thủ công qua GitHub UI làm sót file ẩn như .htaccess hoặc bỏ sót
# thư mục). In luôn ra danh sách file /var/www/html/ để bạn thấy Docker
# đang copy được cái gì.
RUN echo "=== Content of /var/www/html/ after COPY ===" && \
    ls -la /var/www/html/ && \
    echo "=== Verifying required files ===" && \
    for f in index.php config.php .htaccess controllers/SanphamController.php models/database.php views/layout.php public/css/storefront.css; do \
        if [ ! -e "/var/www/html/$f" ]; then \
            ...
            echo "   - Neu upload qua GitHub UI, file an (bat dau bang .) thuong bi sot."; \
            echo "   - Dam bao Render 'Root Directory' duoc set la 'backend'."; \
            exit 1; \
        fi; \
    done && \
